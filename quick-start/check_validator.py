...
OUT_FILE = "./validator_results.csv"
GRAPHS_DIR = "./graphs"
NUM_HOSTS = 2
...
validate["expected"] = ((validate["expected_before"] + validate["expected_after"]) / 2 ) * 2
...
# the difference is the expected value minus the actual median measurement
validate["diff"] = abs((validate["expected"]) - validate["actual_med"])
...
results_scatter = sns.scatterplot(data=validate, x="t", y="diff", hue="OK", style="host")
...
actual_line = sns.lineplot(data=validate, x="t", y="actual_med", hue="sat")
...
# first plot the _expected_ values
reachable_expected = sns.lineplot(data=validate_reachable, x="t", y="expected", hue="sat")
save_fig(reachable_expected, "reachable_expected")

# then plot the actual measurements
reachable_actual = sns.lineplot(data=validate_reachable, x="t", y="actual_med", hue="sat")
