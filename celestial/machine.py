...
class Machine():
    def __init__(
        ...
        self.links: typing.Dict[Machine, typing.Dict[str, typing.Union[int, float]]] = {}
        self.new_links: typing.Dict[Machine, typing.Dict[str, typing.Union[int, float]]] = {}
    ...
    def set_links(self) -> None:
        ...
        for target in self.new_unlinks:
            ...
            if DEBUG:
                print("removing delay from %s %s to %s %s" %(self.shell_no, self.id, target.shell_no, target.id))
        ...
        for target in self.new_links:
            if target in self.links:
                # modify link
                latency_diff = abs(self.new_links[target]["latency"] - self.links[target]["latency"])
                bandwidth_diff = abs(self.new_links[target]["bandwidth"] - self.links[target]["bandwidth"])

                if not latency_diff > LATENCY_UPDATE_THRESHOLD and not bandwidth_diff > BANDWIDTH_UPDATE_THRESHOLD:
                    continue
            ...
            if DEBUG:
                print("modifying delay from %s %s to %s %s to: %d %d" %(self.shell_no, self.id, target.shell_no, target.id, self.new_links[target]["latency"], self.new_links[target]["bandwidth"]))
        ...
        try:
            ...
            print("caught exception while trying to update links for machine %d shell %d:" % (self.id, self.shell_no), e)
    ...
    def __set_active(self) -> None:
        ...
        if not self.active:
            try:
                ...
                self.active = False
                print("caught exception while trying to set machine active %d shell %d:" % (self.id, self.shell_no), e)
    ...
    def __set_inactive(self) -> None:
        ...
        if self.active:
            try:
                ...
                self.active = True
                print("caught exception while trying to set machine inactive %d shell %d:" % (self.id, self.shell_no), e)
